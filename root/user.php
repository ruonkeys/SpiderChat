...
if(isset($_GET['u']))
{
	$u = preg_replace("#[^0-9 a-z]#i", "", $_GET['u']);
}
else
{
...
if($num < 1)
{
	echo "That user does not exist or not yet activated";
	exit();
}
$isOwner = "no";
if($login_username==$u && $user_ok==true)
{
	$isOwner = "yes";
...
if($user_ok==true && $u!=$login_username)
{
	$sql = "select id from friends where user1='$u' and user2='$login_username' and accepted='1' or user1='$login_username' and user2='$u' and accepted='1'";
	$query = mysqli_query($conn, $sql);
	$num = mysqli_num_rows($query);
	if($num > 0)
	{
	...
	$sql = "select id from blockedusers where blocker='$u' and blockee='$login_username'";
	$query = mysqli_query($conn,$sql);
	$num = mysqli_num_rows($query);
	if($num > 0)
	{
	...
	$sql = "select id from blockedusers where blocker='$login_username' and blockee='$u'";
	$query = mysqli_query($conn,$sql);
	$num = mysqli_num_rows($query);
	if($num > 0)
	{
...
else if($user_ok == true && $u != $login_username && $ownerBlockViewer == false)
{
...
else if($user_ok == true && $u != $login_username)
{
...
div.reply_boxes > div > b{font-size:12px;}
    ...
    {
    	var conf = confirm("Please confirm the "+type+" action for user "+user+"");
    	if(conf!=true)
    	{
    		return false;
    	}
