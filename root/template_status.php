...
} else if($isFriend == true && $login_username != $u){
	$status_ui = '<textarea id="statustext" onkeyup="statusMax(this,250)" placeholder="Hi '.$login_username.', say something to '.$u.'"></textarea>';
...
while ($row = mysqli_fetch_array($query, MYSQLI_ASSOC)) {
    ...
    if($replynumrows > 0){
        while ($row2 = mysqli_fetch_array($query_replies, MYSQLI_ASSOC)) {
			...
			$replyDeleteButton = '';
			if($replyauthor == $login_username || $account_name == $login_username ){
	...
	if($isFriend == true || $login_username == $u){
